...
void oplog_printf(const struct fuse_ctx *ctx,const char *format,...) {
	...
	leng = snprintf(buff,LINELENG,"%02u.%02u %02u:%02u:%02u.%06u: uid:%u gid:%u pid:%u cmd:",ltime.tm_mon+1,ltime.tm_mday,ltime.tm_hour,ltime.tm_min,ltime.tm_sec,(unsigned)tv.tv_usec,ctx->uid,ctx->gid,ctx->pid);
	if (leng<LINELENG) {
		va_start(ap,format);
