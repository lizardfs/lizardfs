...
void charts_makechart(uint32_t type,uint32_t range) {
	...
	int32_t i,j;
	uint32_t xy,xm,xd,xh,xminute,xs,xoff,xbold,ys;
	uint64_t max;
	double dmax;
	...
	if (range<VERY_LONG_RANGE) {
		...
		for (i=LENG-xoff-1 ; i>=0 ; i-=xs) {
			if (xh%xbold==0) {
				ys=2;
				if ((range==SHORT_RANGE && xh%6==0) || (range==MEDIUM_RANGE && xh==0) || (range==LONG_RANGE && xd==1)) {
					ys=1;
				}
				if (range<LONG_RANGE) {
					text[0]=xh/10;
