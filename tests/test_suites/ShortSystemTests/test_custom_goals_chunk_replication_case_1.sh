...
assert_equals 20 $(lfscheckfile "${info[mount0]}"/* | grep 'with 2 copies: *1' | wc -l)
...
assert_equals 20 $(lfscheckfile "${info[mount0]}"/* | grep 'with 1 copy: *1' | wc -l)

# Add one "hdd" chunkserver. Expect that second copy of each chunk will be created there.
lizardfs_chunkserver_daemon 2 start
lizardfs_wait_for_ready_chunkservers 2
assert_eventually_prints 20 'lfscheckfile "${info[mount0]}"/* | grep "with 2 copies: *1" | wc -l'
