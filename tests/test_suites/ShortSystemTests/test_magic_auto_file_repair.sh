...
assert_equals 2 $(find_all_chunks | grep -c '_00000001.mfs')
...
MESSAGE="Check if version did increase" assert_equals 2 $(find_all_chunks | grep -c '_00000002.mfs')
...
assert_equals 1 $(egrep -c 'master.fs.file_auto_repaired: [0-9]+ 1' "$TEMP_DIR/log")
file-validate  "$file1" "$file2" # This will result in repairing the chunk from file2
assert_equals 2 $(egrep -c 'master.fs.file_auto_repaired: [0-9]+ 1' "$TEMP_DIR/log")
assert_equals 2 $(egrep -c 'master.fs.file_auto_repaired' "$TEMP_DIR/log")
