...
CHUNKSERVERS=1 \
	USE_RAMDISK=YES \
	MOUNT_EXTRA_CONFIG="lfscachemode=NEVER|lfsioretries=6" \
...
assert_awk_finds '/no valid copies/' "$(lfsfileinfo file)"
...
assert_awk_finds '/[0-9A-F]+_00000002/' "$(lfsfileinfo file)"
...
assert_awk_finds_no '/[0-9A-F]+_00000003/' "$(lfsfileinfo file)"
