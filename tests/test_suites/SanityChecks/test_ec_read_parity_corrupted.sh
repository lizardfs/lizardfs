...
csid=$(find_first_chunkserver_with_chunks_matching 'chunk_ec_3_of_*')
hdd=$(cat "${info[chunkserver${csid}_hdd]}")
chunk=$(find "$hdd" -name 'chunk_ec_3_of_*.???')
echo aaaa | dd of="$chunk" bs=1 count=4 seek=6k conv=notrunc

# Corrupt data in the second parity part of the chunk
csid=$(find_first_chunkserver_with_chunks_matching 'chunk_ec_4_of_*')
hdd=$(cat "${info[chunkserver${csid}_hdd]}")
chunk=$(find "$hdd" -name 'chunk_ec_4_of_*.???')
echo aaaa | dd of="$chunk" bs=1 count=4 seek=6k conv=notrunc
