...
if [[ ! ${DISABLE_PRINTING_XATTRS:-} ]]; then
	...
	find . -type f | sort | while read file; do
		lfsfileinfo "$file" | grep -v $'^\t\t' # remove "copy N" and "no valid copies"
	done
	find . -type f -o -type d | sort | while read file; do
		lfsgetgoal "$file"
		lfsgettrashtime "$file"
		lfsgeteattr "$file"
		lfsdirinfo "$file"
if [[ ! ${DISABLE_PRINTING_XATTRS:-} ]]; then
		getfattr -d "$file"
...
metadata_get_version() {
	lfsmetadump "$1" | awk 'NR==2 {print $6}'
}
...
metadata_generate_trash_ops() {
	touch trashed_file
	if [[ ${LFS_META_MOUNT_PATH-} ]]; then
		# Hack: create files using lfsmakesnapshot so that they will never be opened
		for i in 1 2 3 4 5; do
				lfsmakesnapshot trashed_file trashed_file_$i
		done
		lfssettrashtime 60 trashed_file*
		lfssettrashtime 1 trashed_file_4
		lfssettrashtime 0 trashed_file_5
		...
		echo "untrashed_dir/untrashed_file" > "$LFS_META_MOUNT_PATH"/trash/*trashed_file_3
		mv "$LFS_META_MOUNT_PATH"/trash/*untrashed_file "$LFS_META_MOUNT_PATH"/trash/undel
		mv "$LFS_META_MOUNT_PATH"/trash/*trashed_file_1 "$LFS_META_MOUNT_PATH"/trash/undel
		rm "$LFS_META_MOUNT_PATH"/trash/*trashed_file_2
...
metadata_generate_setgoal() {
	...
	lfssetgoal -r 7 setgoal_recursive

	mkdir -p setgoal_incdec
	for goal in {1..9}; do
		touch setgoal_incdec/setgoal$goal
		lfssetgoal $goal setgoal_incdec/setgoal$goal
	done
	lfssetgoal -r 6- setgoal_incdec
	lfssetgoal -r 3+ setgoal_incdec
}

metadata_generate_settrashtime() {
	for trashtime in 123 234 345 ; do
		touch settrashtime$trashtime
		lfssettrashtime $trashtime settrashtime$trashtime
	done
	...
	lfssettrashtime -r 123456 settrashtime_recursive

	mkdir -p settrashtime_incdec
	lfssettrashtime 100 settrashtime_incdec
	for i in 100 150 200 250 300; do
		touch settrashtime_incdec/file$i
		lfssettrashtime $i settrashtime_incdec/file$i
	done
	lfssettrashtime -r 200+ settrashtime_incdec
	lfssettrashtime -r 200- settrashtime_incdec
}

metadata_generate_seteattr() {
	touch seteattr_dir{1,2,3,4}
	lfsseteattr -f noowner seteattr_dir1
	lfsseteattr -f noattrcache seteattr_dir2
	lfsseteattr -f noentrycache seteattr_dir3
	...
	lfsseteattr -r -f noowner seteattr_recursive
}
...
metadata_generate_chunks() {
	...
	lfsappendchunks chunk_xyz chunk_x chunk_y chunk_z
	truncate -s2 chunk_x
	echo 'zZzZ' >>chunk_z
}
...
metdata_generate_chunks_with_goals() {
	for i in {1..20}; do
		...
		lfssetgoal $i chunks_with_goals_$i
...
metadata_generate_snapshot() {
	...
	lfsmakesnapshot dir_snapshot dir_snapshot_s1

	# Test overwriting shared data
	echo aaaaaaaaaaaa > snapshot_file
	lfsmakesnapshot snapshot_file snapshot_file_s1
	lfsmakesnapshot snapshot_file snapshot_file_s2
	...
	lfsmakesnapshot -o dir_snapshot/level_2/file4 dir_snapshot_2/
	lfsmakesnapshot dir_snapshot dir_snapshot_s2
	lfsmakesnapshot -o dir_snapshot dir_snapshot_s2
}
...
get_changes() {
	find "$1" -regextype posix-egrep -regex '.*/changelog.lfs([.][0-9]+)?$' | xargs sort -n -u
}
