...
ChunkserverWriteChain::~ChunkserverWriteChain() {
	for (const auto& server : servers_) {
		globalChunkserverStats.unregisterWriteOperation(server);
	}
}

void ChunkserverWriteChain::add(const NetworkAddress& server) {
	servers_.push_back(server);
	globalChunkserverStats.registerWriteOperation(server);
}

int ChunkserverWriteChain::connectUsingConnectionPool(ConnectionPool& connectionPool) {
	NetworkAddress chainHead = head();
	int fd = connectionPool.getConnection(chainHead);
	if (fd == -1) {
		fd = createNewChunkserverConnection(chainHead);
	}
	return fd;
}
