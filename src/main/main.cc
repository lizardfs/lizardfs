...
void mainloop() {
	...
	weentry *weit;
	rlentry *rlit;
	struct pollfd pdesc[LFSMAXFILES];
	uint32_t ndesc;
	int i;
	int t,r;
...
int initialize(void) {
	...
	for (i=0 ; (long int)(RunTab[i].fn)!=0 && ok ; i++) {
		...
		try {
			if (RunTab[i].fn() < 0) {
				lfs_arg_syslog(LOG_ERR, "init: %s failed !!!", RunTab[i].name);
				ok=0;
			}
		} catch (const std::exception& e) {
			lfs_arg_syslog(LOG_ERR, "init: %s failed: %s !!!", RunTab[i].name, e.what());
			ok = 0;
		}
	}
	return ok;
...
void changeugid(void) {
	...
	if (geteuid()==0) {
		...
		if (wgroup[0]=='#') {
			...
			if (gr==NULL) {
				lfs_arg_syslog(LOG_WARNING,"%s: no such group !!!",wgroup);
		...
		if (wuser[0]=='#') {
			...
			if (gidok==0) {
				...
				if (pw==NULL) {
					lfs_arg_syslog(LOG_ERR,"%s: no such user id - can't obtain group id",wuser+1);
					exit(LIZARDFS_EXIT_STATUS_ERROR);
				}
				wrk_gid = pw->pw_gid;
			}
			...
			if (pw==NULL) {
				lfs_arg_syslog(LOG_ERR,"%s: no such user !!!",wuser);
				exit(LIZARDFS_EXIT_STATUS_ERROR);
			}
			wrk_uid = pw->pw_uid;
			if (gidok==0) {
		...
		if (setgid(wrk_gid)<0) {
			lfs_arg_errlog(LOG_ERR,"can't set gid to %d",(int)wrk_gid);
			exit(LIZARDFS_EXIT_STATUS_ERROR);
		} else {
			syslog(LOG_NOTICE,"set gid to %d",(int)wrk_gid);
		}
		if (setuid(wrk_uid)<0) {
			lfs_arg_errlog(LOG_ERR,"can't set uid to %d",(int)wrk_uid);
			exit(LIZARDFS_EXIT_STATUS_ERROR);
		} else {
...
FileLock::LockStatus FileLock::wdlock(RunMode runmode, uint32_t timeout) {
	...
	if (ownerpid<0) {
		lfs_errlog(LOG_ERR,"fcntl error");
	...
	if (ownerpid>0) {
		...
		if (runmode==RunMode::kReload) {
			...
			if (kill(ownerpid,SIGHUP)<0) {
				lfs_errlog(LOG_WARNING,"can't send reload signal to lock owner");
				return LockStatus::kFail;
			}
...
#endif
				lfs_errlog(LOG_WARNING,"can't kill lock owner");
				return LockStatus::kFail;
			}
		} else {
		...
		if (*src=='/') {
			*dst='\0';
			if (mkdir(pathbuff,(mode_t)0777)<0) {
				if (errno!=EEXIST) {
					lfs_arg_errlog(LOG_NOTICE,"creating directory %s",pathbuff);
				}
			} else {
				lfs_arg_syslog(LOG_NOTICE,"directory %s has been created",pathbuff);
			}
			*dst++=*src++;
		}
	}
...
int main(int argc,char **argv) {
	...
	movewarning = 0;
	cfgfile=strdup(ETC_PATH "/lfs/" STR(APPNAME) ".cfg");
	passert(cfgfile);
	...
	if (movewarning) {
		lfs_syslog(LOG_WARNING,"default sysconf path has changed - please move " STR(APPNAME) ".cfg from " ETC_PATH "/ to " ETC_PATH "/lfs/");
	}
	...
	if (runmode==RunMode::kStart || runmode==RunMode::kRestart) {
		rls.rlim_cur = LFSMAXFILES;
		rls.rlim_max = LFSMAXFILES;
		if (setrlimit(RLIMIT_NOFILE,&rls)<0) {
			syslog(LOG_NOTICE,"can't change open files limit to %u",LFSMAXFILES);
		}
	...
	if (chdir(wrkdir)<0) {
		lfs_arg_syslog(LOG_ERR,"can't set working directory to %s",wrkdir);
		if (gRunAsDaemon) {
			fputc(0,stderr);
			close_msg_channel();
		}
	...
	if (lockmemory) {
		if (getrlimit(RLIMIT_MEMLOCK,&rls)<0) {
			lfs_errlog(LOG_WARNING,"error getting memory lock limits");
		} else {
			if (rls.rlim_cur!=RLIM_INFINITY && rls.rlim_max==RLIM_INFINITY) {
				rls.rlim_cur = RLIM_INFINITY;
				rls.rlim_max = RLIM_INFINITY;
				if (setrlimit(RLIMIT_MEMLOCK,&rls)<0) {
					lfs_errlog(LOG_WARNING,"error setting memory lock limit to unlimited");
				}
			}
			if (getrlimit(RLIMIT_MEMLOCK,&rls)<0) {
				lfs_errlog(LOG_WARNING,"error getting memory lock limits");
				...
				if (rls.rlim_cur!=RLIM_INFINITY) {
					lfs_errlog(LOG_WARNING,"can't set memory lock limit to unlimited");
				} else {
					if (mlockall(MCL_CURRENT|MCL_FUTURE)<0) {
						lfs_errlog(LOG_WARNING,"memory lock error");
					} else {
						lfs_syslog(LOG_NOTICE,"process memory was successfully locked in RAM");
					}
			}       }
		}
	}
#else
	if (lockmemory) {
		lfs_syslog(LOG_WARNING,"memory lock not supported !!!");
	}
#endif
