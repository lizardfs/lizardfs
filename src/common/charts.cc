...
void charts_load(void) {
	...
	if (fd<0) {
		if (errno!=ENOENT) {
			lzfs_pretty_errlog(LOG_WARNING,"error reading charts data file");
		} else {
			lzfs_pretty_syslog(LOG_NOTICE,"no charts data file - initializing empty charts");
		}
		return;
	}
#ifdef USE_NET_ORDER
	...
	close(fd);
	lzfs_pretty_syslog(LOG_NOTICE,"stats file has been loaded");
	return;
}
