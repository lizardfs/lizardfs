...
TEST(LizardFSVectorTest, GeneralBehaviour) {
	LizardFSVector<int>    vec1_A;
	std::vector<int>      vec1_B;
	EXPECT_EQ(vec1_A, vec1_B);

	LizardFSVector<double> vec2_A(5, 1.0);
	std::vector<double>   vec2_B(5, 1.0);
	EXPECT_EQ(vec2_A, vec2_B);

	LizardFSVector<double> vec3_A(vec2_B);
...
TEST(LizardFSVectorTest, Serialization) {
	typedef std::vector<uint16_t>   OrdinaryVec;
	typedef LizardFSVector<uint16_t> LizardFsVec;
	...
	// Check if LizardFSVector is serialized differently then std::vector:
	ASSERT_NE(o_buffers[0], m_buffers[0]);

	// Check if LizardFSVectors is always serialized the same way:
	ASSERT_EQ(m_buffers[0], m_buffers[1]);
	// Same about std::vector:
	ASSERT_EQ(o_buffers[0], o_buffers[1]);

	LizardFsVec m1_deserialized;
